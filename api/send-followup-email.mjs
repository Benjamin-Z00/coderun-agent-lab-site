...
function buildDay1Email({ name }) {
  ...
  return {
    subject: "CodeRun Agent Lab：今天建议完成 LLM Gateway 的第一步",
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color: #17211b; line-height: 1.7; max-width: 640px;">
        ...
        <p>昨天你领取了 CodeRun Agent Lab 的 LLM Gateway Starter 项目包。今天建议先完成一个很小但关键的目标：把项目在本地跑起来，并确认测试通过。</p>
        <p>这一步的价值不在于代码量，而在于你开始建立一个真实 AI Agent 工程项目的运行、测试和交付闭环。</p>
        <div style="margin: 22px 0; padding: 18px; border: 1px solid #d8ded6; border-radius: 10px; background: #f7f8f5;">
          <h2 style="font-size: 18px; margin: 0 0 10px;">今天建议完成</h2>
          <ol style="margin: 0; padding-left: 20px;">
            <li>打开项目包交付页，进入 GitHub 仓库。</li>
        ...
        <p>如果你已经确定要系统完成后续项目，也可以直接查看完整自学路线：<a href="${PROGRAM_URL}">${PROGRAM_URL}</a></p>
...
function buildDay3Email({ name }) {
  ...
  return {
    subject: "CodeRun Agent Lab：完成免费包后，可以这样继续进阶",
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color: #17211b; line-height: 1.7; max-width: 640px;">
        <p style="margin: 0 0 12px;">${displayName}，你好：</p>
        <p>你领取 LLM Gateway Starter 已经有几天了。如果你已经跑通项目，接下来可以重点思考：这个 Gateway 为什么是 Agent 系统的底层入口。</p>
        <p>如果你还没有开始，也可以先只完成 Day 1 指南。不要追求一次做完，先把本地运行和接口调用跑通。</p>
        <div style="margin: 22px 0; padding: 18px; border: 1px solid #d8ded6; border-radius: 10px; background: #f7f8f5;">
          <h2 style="font-size: 18px; margin: 0 0 10px;">完成后建议继续做</h2>
          <ol style="margin: 0; padding-left: 20px;">
            <li>补全作品集 README。</li>
            <li>说明结构化输出、重试和成本统计的业务价值。</li>
            <li>判断自己是否适合继续做 Tool Runtime、RAG Agent 和 FDE PoC。</li>
          </ol>
        </div>
        <p>
        ...
        <p>免费包不是报名门槛。如果你已经确定要系统学习，可以直接申请完整自学版。</p>
