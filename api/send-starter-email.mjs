  ...
  return {
    ...
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; color: #17211b; line-height: 1.7; max-width: 640px;">
        ...
        <p>这份项目包用于帮助你体验一个真实的 AI Agent 后端工程底座。它是体验入口，不是进入完整自学路线的报名门槛。</p>
        <p>
        ...
        <p>完成后可以提交 GitHub 仓库链接、测试结果和复盘，我们会根据提交内容判断是否需要进一步跟进。你也可以不等完成免费包，直接查看并申请完整自学版。</p>
        <p>
        ...
        <div style="margin-top: 28px; padding: 18px; border: 1px solid #d8ded6; border-radius: 10px; background: #f7f8f5;">
          <h2 style="font-size: 18px; margin: 0 0 8px;">想直接系统学习？</h2>
          <p style="margin: 0 0 14px;">如果你希望系统完成 Tool Runtime、RAG Agent、Codebase Agent 和 FDE PoC 交付包，可以直接查看完整自学路线，并申请首批自学内测。</p>
    ...
    `,
    text: `${rawName}，你好：\n\n感谢你关注 CodeRun Agent Lab。我们已经收到你的项目包领取申请，下面是 LLM Gateway Starter 的交付入口和第一天学习建议。免费包是体验入口，不是进入完整自学路线的报名门槛。\n\n交付页：${STARTER_URL}\nGitHub：${REPO_URL}\n\n第一天建议完成：\n1. Clone 项目并完成本地启动。\n2. 访问 /health 确认服务正常。\n3. 运行 pytest 看测试结果。\n4. 阅读 tasks/01-run-the-service.md，写下 100 字复盘。\n\n完成后可以提交 GitHub 仓库链接、测试结果和复盘：${COMPLETION_FORM_URL}\n\n如果你希望系统完成 Tool Runtime、RAG Agent、Codebase Agent 和 FDE PoC 交付包，也可以直接查看并申请完整自学路线：${PROGRAM_URL}`
  };
}
